...
def dashboard(request):
    if 'user_id' in request.session.keys() and request.session['user_id'] is not None:
        input = request.session['user_id']
        user_details=user.objects.filter(id__iexact=input)
        ...
        return render(request, 'dashboard.html',{'show' : user_details})
    ...
    if request.session and request.session.__contains__('user_id') and request.POST:
        ...
        if obj:
            if request.POST.__contains__('acc_psw') and request.POST['acc_psw']:
                ...
                obj.save()
                return redirect('profile')
            if request.POST.__contains__('acc_name') and request.POST['acc_name']:
               
                obj.user_name = request.POST['acc_name']
                obj.user_keyword = request.POST['acc_keyword']
                obj.user_url = request.POST['acc_url']
                obj.save()
                return redirect('profile')
